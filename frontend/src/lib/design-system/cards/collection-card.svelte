  ...
  {:else}
    <div class="space-y-3">
      <div class="flex items-center justify-between gap-2">
        <span
          class="rounded-full border border-border/70 bg-secondary/35 px-2.5 py-1 text-[11px] uppercase tracking-wide text-muted-foreground"
        >
          {collectionIcon}
        </span>
        <span
          class={`rounded-full px-2 py-0.5 text-[11px] ${riskLabel === "High" ? "bg-destructive/20 text-destructive" : riskLabel === "Medium" ? "bg-yellow-500/20 text-yellow-500" : "bg-green-500/20 text-green-500"}`}
        >
          Risk: {riskLabel}
        </span>
      </div>
      ...
      {#if item.recommendation?.recoverableBytes}
        <p class="text-xs text-muted-foreground">
          Potential reclaim: {Math.round(
            item.recommendation.recoverableBytes / 1024 / 1024 / 1024,
          )} GB
        </p>
      {/if}
