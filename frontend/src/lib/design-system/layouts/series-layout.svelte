<script lang="ts">
  ...
  import type { SeasonObject, SeriesObject } from "$lib/design-system/model/types";
...
<section class="space-y-4 rounded-2xl border border-border/70 bg-card p-4">
  <div class="grid gap-4 md:grid-cols-[220px_1fr]">
    <div class="overflow-hidden rounded-2xl border border-border/70 bg-background/70 aspect-[2/3]">
      {#if series.posterUrl}
        <img src={series.posterUrl} alt={series.title} class="h-full w-full object-cover" loading="lazy" />
      {:else}
        <div class="flex h-full w-full items-center justify-center text-xs text-muted-foreground">No poster</div>
      {/if}
    </div>
    ...
    <div class="space-y-2">
      ...
      <div class="grid gap-2 text-sm text-muted-foreground md:grid-cols-2">
        <p>Affected Seasons: {series.affectedSeasons ?? series.seasons?.length ?? 0}</p>
      ...
      {#if series.recommendation?.explanation}
        <p class="rounded-xl border border-border/60 bg-background/60 p-2 text-xs text-muted-foreground">{series.recommendation.explanation}</p>
      {/if}
    </div>
  </div>
