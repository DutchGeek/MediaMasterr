...
# will use, so a custom DATA_DIR (e.g. /var/mediamasterr) is honoured on restarts.
...
class Settings(BaseSettings):
    ...
    def _persist_generated_secrets(self) -> Settings:
        ...
        if not secrets_path.exists():
            ...
            secrets_path.write_text(
                "# Auto-generated by MediaMasterr on first launch. Do not share.\n"
    ...
    def db_path(self) -> Path:
        """Get database file path with legacy fallback migration support."""
        db_dir = self.data_dir_path / "database"
        db_dir.mkdir(parents=True, exist_ok=True)

        preferred_path = db_dir / "mediamasterr.db"
        legacy_path = db_dir / "reclaimerr.db"
        ...
        if legacy_path.exists():
            try:
                ...
                # Cross-filesystem or permission edge case: copy then continue.
                try:
                    shutil.copy2(legacy_path, preferred_path)
                    return preferred_path
                except OSError:
                    # Last resort keeps existing installs operational.
                    return legacy_path
