...
class FilesystemRoot(Base):
    """Configurable filesystem roots used by MIE discovery and planning."""

    __tablename__ = "filesystem_roots"
...
class MieSettings(Base):
    """Singleton settings row for Media Intelligence Engine behavior."""

    __tablename__ = "mie_settings"
...
class MediaAsset(Base):
    """Unified media asset view used by MIE lifecycle and operations."""

    __tablename__ = "media_assets"
    __table_args__ = (
        UniqueConstraint("media_type", "movie_id", name="uq_media_assets_movie"),
        UniqueConstraint("media_type", "series_id", name="uq_media_assets_series"),
    )
...
class FilesystemIndexEntry(Base):
    """Cached filesystem scanner index entries for MIE."""

    __tablename__ = "filesystem_index_entries"

    id: Mapped[int] = mapped_column(
        Integer, primary_key=True, init=False, autoincrement=True
    )
    root_id: Mapped[int] = mapped_column(
        ForeignKey("filesystem_roots.id", ondelete="CASCADE"), index=True
    )
...
class CleanupPlan(Base):
    """Reviewable batch of operations generated by MIE cleanup planner."""
    ...
    estimated_recovery_bytes: Mapped[int] = mapped_column(Integer, default=0)
...
class CleanupPlanItem(Base):
    """Individual recommendation item inside a cleanup plan."""
    ...
    cleanup_plan_id: Mapped[int] = mapped_column(
        ForeignKey("cleanup_plans.id", ondelete="CASCADE"), index=True
    )
    card_key: Mapped[str] = mapped_column(String(64), index=True)
    title: Mapped[str] = mapped_column(String(255))
    summary: Mapped[str] = mapped_column(String(1024), default="")
    action: Mapped[str] = mapped_column(String(120), default="review")
    safety_level: Mapped[str] = mapped_column(String(32), default="safe")
    ...
    target_path: Mapped[str | None] = mapped_column(String(2048), default=None)
    estimated_recovery_bytes: Mapped[int] = mapped_column(Integer, default=0)
...
class OperationHistory(Base):
    """Audit trail of operations executed from cleanup planning/recommendations."""
